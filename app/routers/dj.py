...
@router.get("/", summary="Get one dad joke.", description="Limit: `3/second`")
@limiter.limit("3/second")
async def get_one(request: Request) -> Dict:
    return {"joke": random.choice(data)}


@router.get(
    "/{amount}", summary="Get multiple dad jokes.", description="Limit: `2/second`"
)
...
async def get_amount(request: Request, amount: int) -> Dict:
    ...
    return {"amount": amount, "jokes": random.sample(data, amount)}
